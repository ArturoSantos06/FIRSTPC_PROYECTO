const AddressCard = ({ address, isSelected, onSelect, onEdit, onDelete, className = '' }) => (
  <div role="radio" aria-checked={isSelected} tabIndex="0" onClick={onSelect} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onSelect(); }} className={`relative flex h-full cursor-pointer flex-col rounded-2xl border-2 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${isSelected ? 'border-[#10B981] ring-4 ring-emerald-50' : 'border-slate-200'} ${className}`}>
