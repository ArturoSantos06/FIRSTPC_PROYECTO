...
const ConfirmOrderView = ({
  address, billing, cartItems, emailQueued, error, isLoading, order, orderNumber,
  paymentName, selectedPaymentMethod, shipping, onNavigate, orderTotal,
}) => (
  <>
    ...
    {order && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="order-success-title"><div className="w-full max-w-md rounded-[32px] border border-emerald-100 bg-white p-6 text-center font-['Montserrat'] shadow-2xl sm:p-10"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 text-3xl text-emerald-600">✓</div><p className="mt-6 text-[11px] font-bold uppercase tracking-[0.3em] text-emerald-500">Pedido confirmado</p><h2 id="order-success-title" className="mt-2 text-3xl font-black text-slate-900">¡Gracias por tu compra!</h2><p className="mt-3 text-sm font-medium text-slate-500">Tu número de orden es <strong className="text-slate-900">{orderNumber}</strong>.</p>{order.distributorOrderId && <p className="mt-2 text-xs font-semibold text-emerald-600">Orden enviada al distribuidor: <strong>{order.distributorOrderId}</strong></p>}<ReceiptPdf order={order} address={address} paymentName={paymentName} shippingName={order.shipping?.carrier || shipping.name} shippingCost={order.shippingCost ?? order.shipping?.cost ?? 0} ivaAmount={order.ivaAmount ?? 0} orderTotal={order.totalPaid ?? 0} /><p className="mt-5 text-xs font-semibold text-slate-500">{emailQueued && 'Hemos enviado una copia de tu comprobante al correo electrónico registrado.'}</p><button type="button" onClick={() => onNavigate('/')} className="mt-5 text-sm font-bold text-emerald-600 hover:text-emerald-700">Volver a la tienda</button></div></div>}
  </>
);
