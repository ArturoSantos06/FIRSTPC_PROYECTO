...
const AdminOrderHistory = () => {
  ...
  const viewReceipt = (order) => openReceiptPdf({ order, address: order.shipping?.address, paymentName: PAYMENT_NAMES[order.paymentMethod] || 'Tarjeta', shippingName: order.shipping?.carrier || 'Envío', shippingCost: order.shipping?.cost ?? order.shippingCost ?? 0, ivaAmount: order.ivaAmount, orderTotal: order.totalPaid });
    ...
    <div className="min-h-screen bg-[#F8FAFC]/50 px-4 pb-12 font-['Montserrat'] pt-28 md:px-6 lg:px-10">
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 rounded-[32px] border border-slate-200/70 bg-white p-6 shadow-[0_24px_70px_rgba(15,23,42,0.06)] sm:p-8">
          ...
          <p className="mt-1 text-sm font-medium text-slate-500">Consulta pedidos y actualiza su estado.</p>
        </header>
