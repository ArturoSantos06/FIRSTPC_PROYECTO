...
const STATUSES = ['Pendiente de pago', 'Procesado', 'Enviado', 'Recibido', 'Cancelado por falta de pago'];
...
const AdminOrderHistory = () => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      const searchable = normalize(`${order.orderNumber} ${getCustomerName(order)} ${getCustomerEmail(order)}`);
      return (!query || searchable.includes(query))
        && (statusFilter === 'all' || (order.status || 'Procesado') === statusFilter);
    });
    ...
    <div className="min-h-screen bg-[#F8FAFC]/50 px-4 pb-12 font-['Montserrat'] pt-28 md:px-6 lg:px-10">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mb-6 flex flex-col gap-3 rounded-[28px] border border-slate-200/70 bg-white p-4 shadow-sm lg:flex-row">
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por folio, nombre o correo..." className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold outline-none transition focus:border-emerald-400 focus:bg-white" />
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold outline-none transition focus:border-emerald-400 lg:w-64"><option value="all">Todos los estados</option>{STATUSES.map((status) => <option key={status}>{status}</option>)}</select>
        </section>
        ...
        <section className="overflow-hidden rounded-[28px] border border-slate-200/70 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.06)]">
          ...
          {!loading && filteredOrders.length > 0 && <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left"><thead className="bg-slate-50 text-[10px] font-black uppercase tracking-[0.16em] text-slate-400"><tr><th className="px-5 py-4">Comprador</th><th className="px-4 py-4">Folio / fecha</th><th className="px-4 py-4">Pago</th><th className="px-4 py-4">Cantidad</th><th className="px-4 py-4">Estado</th><th className="px-5 py-4 text-right">Comprobante</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredOrders.map((order) => { const status = order.status || 'Procesado'; const isOxxoPending = order.paymentMethod === 'oxxo' && status === 'Pendiente de pago'; return <tr key={order.id} className="align-top transition hover:bg-slate-50/70"><td className="px-5 py-4"><p className="max-w-[220px] truncate text-sm font-black text-slate-800" title={getCustomerName(order)}>{getCustomerName(order)}</p><p className="mt-1 max-w-[230px] truncate text-xs font-semibold text-slate-400" title={getCustomerEmail(order)}>{getCustomerEmail(order)}</p></td><td className="px-4 py-4"><p className="text-xs font-black text-slate-800">#{order.orderNumber || order.id}</p><p className="mt-1 whitespace-nowrap text-xs font-semibold text-slate-400">{formatDate(order.createdAt)}</p></td><td className="px-4 py-4"><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-700">{PAYMENT_NAMES[order.paymentMethod] || 'Tarjeta'}</span>{isOxxoPending && <p className="mt-2 whitespace-nowrap text-[11px] font-black text-amber-600">{formatRemaining(order.reservationExpiresAt, now)}</p>}</td><td className="px-4 py-4 text-sm font-black text-slate-700">{getProductQuantity(order)} pieza{getProductQuantity(order) === 1 ? '' : 's'}</td><td className="px-4 py-4"><select value={status} onChange={(event) => handleStatusChange(order, event.target.value)} disabled={savingId === order.id} className={`rounded-xl border px-3 py-2 text-xs font-black outline-none disabled:opacity-50 ${status === 'Pendiente de pago' ? 'border-amber-200 bg-amber-50 text-amber-700' : status.includes('Cancelado') ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'}`}>{STATUSES.map((option) => <option key={option}>{option}</option>)}</select></td><td className="px-5 py-4 text-right"><button type="button" onClick={() => viewReceipt(order)} className="rounded-full bg-emerald-500 px-4 py-2.5 text-xs font-black text-white transition hover:bg-emerald-600">Ver PDF</button></td></tr>; })}</tbody></table></div>}
        </section>
      </div>
